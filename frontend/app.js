  ...
  document.getElementById("tasks").innerHTML =
    tasks
      .map((t) => {
        ...
        return `<li class="${done ? "done" : ""}">${t.id} ${title} [user ${t.user_id}] ${done ? "Completed" : "Pending"} <button onclick="toggleTask(${t.id},${!done})">${done ? "Not Complete" : "Complete"}</button> <button onclick="delTask(${t.id})">Delete</button></li>`;
      })
